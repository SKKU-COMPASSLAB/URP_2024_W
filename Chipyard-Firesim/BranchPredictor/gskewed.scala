...
case class BHTParams(
  nEntries: Int = 1024,
  counterLength: Int = 2,
  historyLength: Int = 10, //log2(nEntries)
  historyBits: Int = 4)
...
class BHTResp(implicit p: Parameters) extends BtbBundle()(p) {
  ...
  val value00 = UInt(btbParams.bhtParams.map(_.counterLength).getOrElse(1).W)
...
class BHT(params: BHTParams)(implicit val p: Parameters) extends HasCoreParameters {
  ...
  def index0(addr: UInt, history: UInt) = {
    def H(x: UInt) = {
      val n = x(log2Up(params.nEntries)-1) ^ x(0)
      val s = x(log2Up(params.nEntries)-1, 1)
      val h = Cat(n, s)
      h
    }
    ...
    def hashHistory(hist: UInt) = if (params.historyLength == params.historyBits) hist else {
      ...
      (i.U * hist)(params.historyLength-1, params.historyLength-params.historyBits) //결과는 historyBits만큼 return
    }
    def hashAddr(addr: UInt) = {
      val hi = addr >> log2Ceil(fetchBytes)
      hi(log2Ceil(params.nEntries)-1, 0) ^ (hi >> log2Ceil(params.nEntries))(1, 0) // return되는 값의 크기는 log2(nEntries)
    }
    hashAddr(addr) ^ (hashHistory(history) << (log2Up(params.nEntries) - params.historyBits)) // return되는 값의 크기는 log2(nEntries)
    */
  }
  ...
  def get(addr: UInt): BHTResp = {
    ...
    val a = ((value0(1) & value1(1)) | (value1(1) & value2(1)) | (value2(1) & value0(1)))
  ...
  private val table0 = Mem(params.nEntries, UInt(params.counterLength.W))
  ...
  when (wen) { table0(waddr0) := wdata0 }
  when (wen) { table1(waddr1) := wdata1 }
  when (wen) { table2(waddr2) := wdata2 }
}
