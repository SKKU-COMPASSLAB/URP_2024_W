...
class BHT(params: BHTParams)(implicit val p: Parameters) extends HasCoreParameters {
  ...
  def get(addr: UInt): BHTResp = {  
    ...
    }.otherwise (res.value := Mux(resetting, 3.U, table(index(addr, history)) << 1))
    ...
    printf(cf"u1 = $u1\n u2 = $u2\n u3 = $u3\n u4 = $u4\n")
      ...
      when ((tagMatch44 === 1.U)) {
        ...
        when (tagMatch33 === 1.U) {
          ...
          when (table(index(addr, d.history))(1) =/= T4_pred(index4(addr, d.history))(2)){
            wen_T4_u := true.B
            wen_T3_u := false.B
            wen_T2_u := false.B
            wen_T1_u := false.B
        ...
        when (tagMatch22 === 1.U) {
          ...
          when (table(index(addr, d.history))(1) =/= T3_pred(index3(addr, d.history))(2)){
            wen_T4_u := false.B
            wen_T3_u := true.B
            wen_T2_u := false.B
            wen_T1_u := false.B
        ...
        when (tagMatch11 === 1.U) {
          ...
          when (table(index(addr, d.history))(1) =/= T2_pred(index2(addr, d.history))(2)){
            wen_T4_u := false.B
            wen_T3_u := false.B
            wen_T2_u := true.B
            wen_T1_u := false.B
        ...
        when (table(index(addr, d.history))(1) =/= T1_pred(index1(addr, d.history))(2)){
          wen_T4_u := false.B
          wen_T3_u := false.B
          wen_T2_u := false.B
          wen_T1_u := true.B
        ...
        wdata := Cat(d.value === 4.U || d.value(1) && taken, taken ^ d.value(2))
  ...
  private val table = Mem(params.nEntries, UInt(2.W))
