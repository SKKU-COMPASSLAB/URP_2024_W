...
object ReplacementPolicy {
  def fromString(s: String, n_ways: Int): ReplacementPolicy = s.toLowerCase match {
    ...
    case "fifo"   => new FirstInFirstOut(n_ways)
    case "mplru"  => new ModifiedPseudoLRU(n_ways)
...
class FirstInFirstOut(n_ways: Int) extends ReplacementPolicy {
  // Example 4-way (2-bits):
  // [11] - way to be replaced = 3
  // [10] - way to be replaced = 2
  // [01] - way to be replaced = 1
  // [00] - way to be replaced = 0
  private val replace = Wire(Bool())
  replace := false.B

  def nBits = log2Up(n_ways)
  def perSet = true
  private val state_reg = RegInit(0.U(nBits.W))
  def state_read = WireDefault(state_reg)

  def get_next_state(state: UInt, touch_way: UInt): UInt = {
    val nextState = Wire(UInt(nBits.W))
    when (replace) {
      nextState := state + 1.U
    }.otherwise {
      nextState := state
  ...
  def access(touch_ways: Seq[Valid[UInt]]): Unit = {
    ...
    for (i <- 1 until touch_ways.size) {
      cover(PopCount(touch_ways.map(_.valid)) === i.U, s"FIFO_UpdateCount$i", s"FIFO Update $i simultaneous")
    }
  }

  def get_replace_way(state: UInt): UInt = state_reg
...
class PseudoLRU(n_ways: Int) extends ReplacementPolicy {
  ...
  def nBits = n_ways - 1   // 4-way니까 3bit
  def perSet = true
...
class ModifiedPseudoLRU(n_ways: Int) extends ReplacementPolicy {
  
  private def getTBAI(nways: Int): Int = if (nways == 1) 0 else (nways + 1) >> 1
  private def getMBAI(nways: Int): Int = if (nways == 1) 0 else (nways - 1 - getTBAI(nways)) * 2

  def nBits = {
    val nTBAI = getTBAI(n_ways)
    val nMBAI = getMBAI(n_ways)
    nTBAI + nMBAI
  }
  def perSet = true
  private val state_reg = if (nBits == 0) Reg(UInt(0.W)) else RegInit(0.U(nBits.W))
  ...
  private def extractcurrentstate(state: UInt, nTBAI: Int, nMBAI: Int): UInt = {

    if (nTBAI > 1) {
        val right_TBAI: Int = 1 << (log2Ceil(nTBAI) - 1)
        val right_MBAI: Int = 2 * (right_TBAI - 1)
        val right_subtree_state = state(right_TBAI + right_MBAI - 1, 0)
        if (right_MBAI == nMBAI) {   // the root is TBAI, so there is only right tree
          Cat(state(nTBAI + nMBAI - 1), extractcurrentstate(right_subtree_state, right_TBAI, right_MBAI))
        } else {     // the root is MBAI, so there are right tree and left tree
          val left_TBAI: Int = nTBAI - right_TBAI
          val left_MBAI: Int = nMBAI - 2 - right_MBAI
          val left_subtree_state  = state(nTBAI + nMBAI - 3, right_TBAI + right_MBAI)
          Cat(state(nTBAI + nMBAI - 1), 
              extractcurrentstate(left_subtree_state, left_TBAI, left_MBAI), 
              extractcurrentstate(right_subtree_state, right_TBAI, right_MBAI))
        }
    } else if (nTBAI == 1) {
      state(0)
    } else {  // tree_nways <= 1
  ...
  private def extractpreviousstate(state: UInt, nTBAI: Int, nMBAI: Int): UInt = {
    ...
    if (nTBAI > 1) {
        ...
        val right_subtree_state = state(right_TBAI + right_MBAI - 1, 0)
        if (right_MBAI == nMBAI) {   // the root is TBAI, so there is only right tree
          Cat(state(nTBAI + nMBAI - 1), extractpreviousstate(right_subtree_state, right_TBAI, right_MBAI))
        } else {    // the root is MBAI, so there are right tree and left tree
          val left_TBAI: Int = nTBAI - right_TBAI
          val left_MBAI: Int = nMBAI - 2 - right_MBAI
          val left_subtree_state  = state(nTBAI + nMBAI - 3, right_TBAI + right_MBAI)
          Cat(state(nTBAI + nMBAI - 2), 
              extractpreviousstate(left_subtree_state, left_TBAI, left_MBAI), 
              extractpreviousstate(right_subtree_state, right_TBAI, right_MBAI))
        }
    } else if (nTBAI == 1) {
      state(0)
    } else {  // tree_nways <= 1
  ...
  private def getState(current_state: UInt, previous_state: UInt, nTBAI: Int, nMBAI: Int): UInt = {
    if (nTBAI > 1) {
        val right_TBAI = 1 << (log2Ceil(nTBAI) - 1)
        val right_MBAI = 2 * (right_TBAI - 1)
        val right_subtree_current_state = current_state(right_TBAI + right_MBAI / 2 - 1, 0)
        val right_subtree_previous_state = previous_state(right_TBAI + right_MBAI / 2 - 1, 0)
        if (right_MBAI == nMBAI) {   // the root is TBAI, so there is only right tree
          Cat(current_state(nTBAI + nMBAI / 2 - 1), getState(right_subtree_current_state, right_subtree_previous_state, right_TBAI, right_MBAI))
        } else {    // the root is MBAI, so there are right tree and left tree
          val left_TBAI = nTBAI - right_TBAI
          val left_MBAI = nMBAI - 2 - right_MBAI
          val left_subtree_current_state  = current_state(nTBAI + nMBAI / 2 - 2, right_TBAI + right_MBAI / 2) 
          val left_subtree_previous_state  = previous_state(nTBAI + nMBAI / 2 - 2, right_TBAI + right_MBAI / 2)
          Cat(current_state(nTBAI + nMBAI / 2 - 1), 
              previous_state(nTBAI + nMBAI / 2 - 1), 
              getState(left_subtree_current_state, left_subtree_previous_state, left_TBAI, left_MBAI), 
              getState(right_subtree_current_state, right_subtree_previous_state, right_TBAI, right_MBAI))
        }
    } else if (nTBAI == 1) {
      current_state(0)
    } else {  // tree_nways <= 1
  ...
  def get_next_state(state: UInt, touch_way: UInt): UInt = {
    ...
    val previous_state = extractcurrentstate(state, getTBAI(n_ways), getMBAI(n_ways))
    val current_state = get_next_state(previous_state, touch_way_sized, n_ways)
    val current_previous_state = getState(current_state, previous_state, getTBAI(n_ways), getMBAI(n_ways))
  ...
  def get_replace_way(state: UInt): UInt = get_replace_way(extractpreviousstate(state, getTBAI(n_ways), getMBAI(n_ways)), n_ways)
...
class ModifiedPseudoLRU(n_ways: Int) extends ReplacementPolicy {
  ...
  private def extractcurrentstate(state: UInt, nTBAI: Int, nMBAI: Int): UInt = {
    ...
    if (nTBAI > 1) {
      if (nTBAI == 2 && nMBAI == 2) {   // TBAI = 2, MBAI = 1 * 2, nways = 4
        Cat(state(nTBAI + nMBAI - 1), state(1, 0))
      } else {
        val right_TBAI: Int = 1 << (log2Ceil(nTBAI) - 1)
        val right_MBAI: Int = 2 * (right_TBAI - 1)
        val right_subtree_state = state(right_TBAI + right_MBAI - 1, 0)
        if (right_MBAI == nMBAI) {   // the root is TBAI, so there is only right tree
          ...
          val left_TBAI: Int = nTBAI - right_TBAI
          val left_MBAI: Int = nMBAI - 2 - right_MBAI
          val left_subtree_state  = state(nTBAI + nMBAI - 3, left_TBAI + left_MBAI)
          Cat(state(nTBAI + nMBAI - 1), 
              extractcurrentstate(left_subtree_state, left_TBAI, left_MBAI), 
              extractcurrentstate(right_subtree_state, right_TBAI, right_MBAI))
        }
  ...
  private def extractpreviousstate(state: UInt, nTBAI: Int, nMBAI: Int): UInt = {
    ...
    if (nTBAI > 1) {
      if (nTBAI == 2 && nMBAI == 2) {   // TBAI = 2, MBAI = 1 * 2, nways = 4
        Cat(state(nTBAI + nMBAI - 2), state(1, 0))
      } else {
        val right_TBAI: Int = 1 << (log2Ceil(nTBAI) - 1)
        val right_MBAI: Int = 2 * (right_TBAI - 1)
        val right_subtree_state = state(right_TBAI + right_MBAI - 1, 0)
  ...
  private def getState(current_state: UInt, previous_state: UInt, nTBAI: Int, nMBAI: Int): UInt = {
    if (nTBAI > 1) {
      if (nTBAI == 2 && nMBAI == 2) {   // TBAI = 2, MBAI = 1 * 2, nways = 4
        Cat(current_state(nTBAI + nMBAI / 2 - 1), previous_state(nTBAI + nMBAI / 2 - 1),  current_state(1, 0))
      } else {
        val right_TBAI = 1 << (log2Ceil(nTBAI) - 1)
        val right_MBAI = 2 * (right_TBAI - 1)
        if (right_MBAI == nMBAI) {   // the root is TBAI, so there is only right tree
          Cat(current_state(nTBAI + nMBAI / 2 - 1), getState(current_state(nTBAI + nMBAI / 2 - 2, 0), previous_state(nTBAI + nMBAI / 2 - 2, 0), nTBAI - 1, nMBAI))
        } else {    // the root is MBAI, so there are right tree and left tree
          val left_TBAI = nTBAI - right_TBAI
          val left_MBAI = nMBAI - 2 - right_MBAI
          val left_subtree_current_state  = current_state(nTBAI + nMBAI / 2 - 2, right_TBAI + right_MBAI / 2)
  ...
  def access(touch_ways: Seq[Valid[UInt]]): Unit = {
    ...
    for (i <- 1 until touch_ways.size) {
      cover(PopCount(touch_ways.map(_.valid)) === i.U, s"MPLRU_UpdateCount$i", s"MPLRU Update $i simultaneous")
    }
  }
  ...
  /** @param state state_reg bits for this sub-tree
    ...
    */
  def get_next_state(state: UInt, touch_way: UInt, tree_nways: Int): UInt = {
    val currentstate: UInt = extractcurrentstate(state, getTBAI(tree_nways), getMBAI(tree_nways))

    require(currentstate.getWidth == (tree_nways-1),                   s"wrong state bits width ${currentstate.getWidth} for $tree_nways ways")
    require(touch_way.getWidth == (log2Ceil(tree_nways) max 1), s"wrong encoded way width ${touch_way.getWidth} for $tree_nways ways")
    ...
    if (tree_nways > 2) {
      ...
      val left_nways:  Int = tree_nways - right_nways         // number of ways in the left sub-tree
      val set_left_older      = !touch_way(log2Ceil(tree_nways)-1)
      val left_subtree_state  = currentstate.extract(tree_nways-3, right_nways-1)
      val right_subtree_state = currentstate(right_nways-2, 0)
  ...
  def get_next_state(state: UInt, touch_way: UInt): UInt = {
    ...
    val previous_state = extractcurrentstate(state, getTBAI(n_ways), getMBAI(n_ways))
    val current_state = get_next_state(state, touch_way_sized, n_ways)
    val current_previous_state = getState(current_state, previous_state, getTBAI(n_ways), getMBAI(n_ways))
  ...
  /** @param state state_reg bits for this sub-tree
    ...
    */
  def get_replace_way(state: UInt, tree_nways: Int): UInt = {
    val previousstate: UInt = extractpreviousstate(state, getTBAI(tree_nways), getMBAI(tree_nways))

    require(previousstate.getWidth == (tree_nways-1), s"wrong state bits width ${previousstate.getWidth} for $tree_nways ways")
    ...
    if (tree_nways > 2) {
      ...
      val right_nways: Int = 1 << (log2Ceil(tree_nways) - 1)  // number of ways in the right sub-tree
      val left_nways:  Int = tree_nways - right_nways         // number of ways in the left sub-tree
      val left_subtree_older  = previousstate(tree_nways-2)
      val left_subtree_state  = previousstate.extract(tree_nways-3, right_nways-1)
      val right_subtree_state = previousstate(right_nways-2, 0)
